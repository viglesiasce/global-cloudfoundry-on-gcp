...
pushd /global-cloudfoundry-on-gcp/central/bosh
  export HOME=/root/
  bosh-init deploy manifest.yml

  printf "admin\nadmin\n" | bosh target https://10.0.0.6:25555 micro-google
...
pushd /global-cloudfoundry-on-gcp/central/cloudfoundry
  ...
  export ADDRESS=`gcloud compute addresses list --format json cf-us-central1 | jq -r '.[0].address'`
  export DOMAIN=central.cf.gcp.solutions
